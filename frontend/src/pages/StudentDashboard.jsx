...
const StudentDashboard = () => {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      console.error(err);
      setError('Failed to load complaints. Please check server connection.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Track your issues, comments, and resolutions in real-time</p>
        </div>
        <Link
          to="/complaints/new"
          className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm shadow-md hover:shadow-lg transition-all duration-200 cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Claims', value: stats.total, color: 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/20' },
          { label: 'Unassigned/Pending', value: stats.pending, color: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20' },
          { label: 'In Progress', value: stats.inProgress, color: 'text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/20' },
          { label: 'Resolved Tickets', value: stats.resolved, color: 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20' }
        ].map((card, idx) => (
          <div key={idx} className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 shadow-xs flex items-center justify-between">
            <div>
      ...
      <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 shadow-xs flex flex-col md:flex-row gap-4 items-center justify-between">
        {/* Search */}
      ...
      {loading ? (
        ...
        <div className="p-12 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl bg-white dark:bg-slate-900 shadow-2xs">
          ...
          <h3 className="font-bold text-slate-700 dark:text-slate-300 text-lg">No Complaints Found</h3>
          <p className="text-slate-400 dark:text-slate-500 text-sm mt-1 mb-5 max-w-sm mx-auto">
            {searchTerm || statusFilter || categoryFilter
              ? "No tickets match your filter criteria. Try clearing search keywords or category filters."
              : "You haven't registered any complaints. Click the button below to submit your first issue!"}
          </p>
          <Link
            to="/complaints/new"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm shadow-md transition-all cursor-pointer"
          >
            <Plus className="w-4 h-4" />
            File First Issue
          </Link>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredComplaints.map((complaint) => (
            ...
            >
              ...
              {/* Card Footer */}
              <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/60 mt-4 pt-3 text-[10px] text-slate-400 dark:text-slate-500">
                ...
                {complaint.staff ? (
                  ...
                  <span className="italic flex items-center gap-1 text-amber-600 dark:text-amber-500 font-medium">
                    <Clock className="w-3 h-3" /> Waiting Admin
                  </span>
                )}
              </div>
            </Link>
